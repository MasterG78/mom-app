-- Migration: Make RPC parameters optional for upsert_invoice_line_item
-- Description: Adds DEFAULT NULL to p_quantity, p_rate, and p_amount to handle 
--              cases where Make.com/QBO omits fields (often when they are null).

-- 1. Drop existing version to ensure signature update
DROP FUNCTION IF EXISTS public.upsert_invoice_line_item(text, text, text, text, text, text, text);

-- 2. Create updated version with optional parameters
